...
class JsonSchemaValidator implements MiddlewareInterface
{
     ...
     *
     * @param ValidationType $type Source of data.
     ...
     */
    protected function getData(ValidationType $type, ServerRequestInterface $request): object
    {
        return match ($type) {
            ValidationType::RequestBody => $this->getRequestBodyData($request),
            ValidationType::QueryParameters => $this->getQueryParametersData($request),
        };
    }
    ...
    private function getQueryParametersData(ServerRequestInterface $request): object
    {
        ...
        $encoded = json_encode($queryParams, JSON_THROW_ON_ERROR);
     ...
     *
     * @param ValidationType $type The validation target type.
     * @param string $schemaName Name of the schema to load.
     *
     * @return object JSON schema object.
     */
    protected function getSchema(ValidationType $type, string $schemaName): object
    {
        $prefix = match ($type) {
            ValidationType::RequestBody => 'RequestBody/',
            ValidationType::QueryParameters => 'QueryParameters/',
        };
    ...
    {
        $modifiedName = preg_replace('/([a-z])([A-Z])|([A-Z])([A-Z][a-z])/', '$1$3_$2$4', $variableName);
     ...
     * Internal use only — called when schema validation fails during process().
     *
     * @param SchemaValidationResult $result The failed validation result.
     *
     * @throws JsonException If JSON encoding fails.
     *
     * @return ResponseInterface A JSON error response with HTTP 400 status.
     */
    private function errorHandler(SchemaValidationResult $result): ResponseInterface
    {
        $errors = [];

        foreach ($result->errors as $key => $errorArray) {
        ...
        $encodedPayload = json_encode($payload, Payload::JSON_FLAGS);
